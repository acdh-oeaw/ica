...
test.describe("app", () => {
	...
	test("should add aria-current attribute to nav links", async ({ createIndexPage, page }) => {
		...
		const homeLink = indexPage.page
			...
			.first();
		const link = indexPage.page.getByRole("link", {
			name: i18n.t("AppHeader.links.geo-visualisation"),
		});
		...
		await link.click();
		await page.waitForURL("**/geo-visualisation");
